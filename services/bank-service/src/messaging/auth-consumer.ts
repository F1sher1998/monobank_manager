import { AUTH_EVENT_EXCHANGE, BANK_USER_CREATED_ROUTING_KEY, type AuthBankUserPayloadEvent } from "@common/src";
...
const handleMessage = async(message: ConsumeMessage, ch: Channel) => {
        ...
        const event = JSON.parse(raw) as AuthBankUserPayloadEvent;

        await bankUserService.AuthSyncUser(event.payload);
...
export const startAuthEventConsumer = async () => {
        ...
        const connection = (await connect(env.RABBITMQ_DB_URL!)) as ManagedConnection;
        ...
        await ch.bindQueue(queue.queue, AUTH_EVENT_EXCHANGE, BANK_USER_CREATED_ROUTING_KEY);
